	...
	class product // chau dep trai quá
	{
		...
		public function insert_product($date,$files){
			...
			if($productName == "" || $productQuantity == "" || $category == "" || $brand == "" || $product_desc == "" || $price == "" || $type == "" || $file_name == ""){
				...
				$query = "INSERT INTO tbl_product(productName,product_code,product_remain,productQuantity,catId,brandId,product_desc,price,image,type) VALUES('$productName','$product_code','$productQuantity','$productQuantity','$category','$brand','$product_desc','$price','$unique_image','$type') ";
		...
		public function getproduct_xemay(){
			$query = "SELECT * FROM tbl_product WHERE catId = '19' order by productId desc LIMIT 4 ";
			$result = $this->db->select($query);
			return $result;
		}
		public function getproduct_moto(){
			$query = "SELECT * FROM tbl_product WHERE catId = '21' order by productId desc LIMIT 4 ";
			$result = $this->db->select($query);
			return $result;
		}
		public function getproduct_oto(){
			$query = "SELECT * FROM tbl_product WHERE catId = '20' order by productId desc LIMIT 4 ";
			$result = $this->db->select($query);
			return $result;
		}
